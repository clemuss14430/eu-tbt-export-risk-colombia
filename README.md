...
│
├── Maestro_Riesgo_TBT_ePing.xlsx                      # Master output file
│
├── Annex_B_ICS_HS_Equivalence_Dictionary.md       # Annex B — complete ICS–HS mapping
│
└── README.md
```
...
The complete **ICS–HS Equivalence Dictionary** used in Script 02 is provided as **Annex B (online supplementary material)**:

**[Annex B — ICS–HS Equivalence Dictionary Used in the ePing Alert Homologation Script](Annex_B_ICS_HS_Equivalence_Dictionary.md)**

The annex documents the equivalence rules used to translate **International Classification for Standards (ICS)** codes into **Harmonized System (HS)** chapters when ePing notifications do not contain a directly identifiable tariff code.

The dictionary is part of the reproducibility framework of the study. It allows the ICS-to-HS homologation stage to be independently reviewed and replicated without embedding the complete technical table in the working paper.

> **Methodological note:** 83.5% of processed ePing notifications (86 of 103) relied on the ICS-to-HS equivalence dictionary rather than a direct HS code. The complete mapping rules are therefore made available in Annex B.

General environmental classifications without a specific product correspondence are coded as **`ENV-General`**. These records are retained for regulatory traceability but do not generate product-level cross-checking with export flows.
...
| `Risk_Score`             | Integer     | Numeric RPI score                                                         |
...
* Applies the **ICS-to-HS equivalence dictionary documented in Annex B** for notifications without a direct tariff code
...
### ICS-to-HS homologation and classification bias

The ICS-to-HS equivalence dictionary is a methodological component of Script 02 rather than a standalone descriptive table. Because 86 of the 103 processed records (83.5%) required ICS-based homologation, the resulting HS assignment depends on the equivalence rules documented in **Annex B**.

Where an ICS category corresponds to a broad range of HS chapters, the range is expanded by the integration script. This can increase the breadth of product exposure assigned to a notification and may reduce product-level specificity. The issue is particularly relevant to the 35 records mapped to Chapters 28–38.

General regulatory classifications such as environmental-management frameworks are retained as `ENV-General` for traceability but do not generate product-level export matches.
...
The repository is structured so that the principal analytical outputs and the methodological mapping required to reproduce the ePing homologation stage are available together.

**Primary analytical resources:**

* **Scripts 01–03:** complete processing pipeline
* **`Maestro_Riesgo_TBT_ePing.xlsx`:** processed master dataset and ePing bridge table
* **Power BI dashboard:** interactive visualization of the analytical results
* **Annex B:** complete ICS–HS equivalence dictionary used by Script 02

The raw ODEB, I-TIP and ePing source files are not redistributed because of size and licensing constraints. The corresponding public source platforms are identified in the **Data sources** section.

---

## Suggested citation

If you use this repository, the master file, its scripts, or the supplementary Annex B in your research, please cite:

### Repository

> Lemus Serna, C. I. (2026). *eu-tbt-export-risk-colombia* [Source code repository]. GitHub.
> https://github.com/clemuss14430/eu-tbt-export-risk-colombia

### Dashboard

> Lemus, C. (2026). *Latent Risk and Regulatory Persistence: OTC-ePing Trade Intelligence Dashboard for Colombian Manufacturing Exports to the European Union* [Interactive Dashboard, Microsoft Power BI]. Universidad EAN.
> https://app.powerbi.com/view?r=eyJrIjoiNDkzZGVlYjMtZjZiNi00ODFmLWE4MDEtMzRhMmRlODRlMTk3IiwidCI6ImMwNmZiNTU5LTFiNjgtNGI4NC1hMTRmLTQ3ZDBkODM3YTVhYiIsImMiOjR9&pageName=3f6bdce00eefc57e3076
